...
import { AmbientBackground } from './SolarScene'
...
function Logo({ onClick }) {
  ...
  return (
    <button onClick={onClick} className="flex items-center gap-2.5 group" aria-label={t.brand}>
      <span className="relative grid place-items-center h-9 w-9 rounded-xl" style={{ background: 'linear-gradient(135deg, #2f6bff, #ffd60a)' }}>
        <Icon.sun className="h-5 w-5 text-[#060912]" />
        <span className="absolute inset-0 rounded-xl shimmer opacity-60" />
      </span>
      <span className="font-display font-extrabold text-[1.05rem] tracking-tight leading-none">
        {t.brand}
      </span>
    </button>
  )
